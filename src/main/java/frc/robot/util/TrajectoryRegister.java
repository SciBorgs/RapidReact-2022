 ...
 * <p>
 * For use, set the field with TrajectoryRegister.setField2d().
 */
public class TrajectoryRegister {
    private static final List<String> slots = Collections.unmodifiableList(List.of(
        "traj-white", "traj-red", "traj-blue", "traj-yellow", "traj-green", "traj-orange", "traj-purple"
    ));
    ...
    private static SendableChooser<Trajectory> getTrajectoryChooser() {
        ...
        chooser.setDefaultOption("None", PathPlanner.loadPath(names.get(0), 0, 0).transformBy(new Transform2d(new Translation2d(100, 100), new Rotation2d())));
        for (String name : names) {
            chooser.addOption(name, PathPlanner.loadPath(name, DriveConstants.maxVel, DriveConstants.maxAccel));
        }
        return chooser;
    }
