...
public class CombinedCorrection<PIDModel> {
  ...
  private void check(PIDModel checkback, double tolr) throws IllegalStateException {
    if (checkback instanceof PIDController) {
      ...
      throw new IllegalStateException(
          "<PIDModel> of CombinedCorrection is not PIDController or ProfiledPIDController. This is not allowed.");
  }

  private void check(PIDModel checkback) {
    if (checkback instanceof PIDController) type = correctionType.VELOCITY;
    else if (checkback instanceof ProfiledPIDController) type = correctionType.DISTANCE;
    else
      throw new IllegalStateException(
          "<PIDModel> of CombinedCorrection is not PIDController or ProfiledPIDController. This is not allowed.");
  }
  ...
  public double getVoltage(double proccess, double setPoint) {
    if (type == correctionType.DISTANCE) {
      ...
      // now pid and ff are used to try and reach this velocity as close as possible since it is
      // assumed that profilepid is correct in required velocity to reach target
      return ((ProfiledPIDController) feedback)
              .calculate(proccess, new TrapezoidProfile.State(setPoint, 0))
          + feedforward.calculate(
              velocity); // assume that you want to reach setPoint with no remaining velocity
    } else
      return ((PIDController) feedback).calculate(proccess, setPoint)
  ...
  public double getVoltage(double proccess, double setPoint, double setAcceleration) {
    if (type == correctionType.DISTANCE) {
      ...
      return ((ProfiledPIDController) feedback)
          ...
          + feedforward.calculate(
              velocity,
              setAcceleration); // assume that you want to reach setPoint with no remaining velocity
    } else
      return ((PIDController) feedback).calculate(proccess, setPoint)
          + feedforward.calculate(setPoint, setAcceleration); // acceleration
  }
